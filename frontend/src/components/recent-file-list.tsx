...
export function RecentFileList() {
  ...
  const fetchFiles = async () => {
    ...
    try {
      ...
      const data = await documentsApi.getRecentFiles();
  ...
  const handleToggleStar = async (id: string, fileName: string, isCurrentlyStarred: boolean) => {
    try {
      ...
      setFiles((prev) =>
        prev.map((f) =>
          f.id === id ? { ...f, isStarred: !isCurrentlyStarred } : f
        )
      );
      ...
      toast({
        ...
        description: `${fileName} has been ${isCurrentlyStarred ? "unstarred" : "starred"}.`,
      });
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "Star operation failed",
    ...
    <div className="rounded-xl border bg-card text-card-foreground shadow-sm">
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4">
          <div className="flex h-20 w-20 items-center justify-center rounded-full bg-blue-500/10 border-2 border-blue-500/20 mb-4">
            <Clock className="h-10 w-10 text-blue-500" />
          </div>
          <p className="text-lg font-medium mb-1">No recent files</p>
          <p className="text-sm text-muted-foreground text-center max-w-sm">
            Files you've accessed in the last 30 days will appear here for quick access.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b bg-muted/50 text-xs font-medium uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="hidden px-4 py-3 lg:table-cell">Last Accessed</th>
                <th className="hidden px-4 py-3 xl:table-cell">Uploaded</th>
                ...
                >
                  ...
                  <td className="hidden px-4 py-3 text-muted-foreground lg:table-cell">
                    <div className="flex flex-col">
                      <span>{formatDistanceToNow(new Date(file.lastAccessedAt), { addSuffix: true })}</span>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button
                        ...
                        onClick={() => handleToggleStar(file.id, file.fileName, file.isStarred)}
                        title={file.isStarred ? "Remove from starred" : "Add to starred"}
                      >
                        <Star className={`h-4 w-4 ${file.isStarred ? "fill-yellow-500" : ""}`} />
                      </Button>
                      <Button
                        variant="ghost"
                        size="icon"
